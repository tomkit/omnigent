...
def test_fork_bundle_loads() -> None:
    """The fork bundle parses — a broken bundle silently disables the agent."""
...
def test_bundle_does_not_point_at_the_examples_tree() -> None:
    ...
    """
    config = (_POLLY / "config.yaml").read_text(encoding="utf-8")
    assert "examples/polly" not in config
    assert "deploy/agents/polly/skills" in config
