...
def _resolve_provider_for_build(
    ...
    *,
    ...
    if model is not None and model.startswith(("databricks-", "databricks/")):
        return None
    detected = effective_config_with_detected(explicit_config)
    ...
    if harness_type == "pi" and model is not None:
        ...
        by_model = get_default_provider(detected, preferred)
        if by_model is not None:
            return by_model
    return default_provider_for_harness(detected, harness)
