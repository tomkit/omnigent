...
> fork's judge-parser scan was re-applied on top for v0.9.0–v0.11.0, then
> dropped again at v0.14.0 (2026-09-19) to reach zero upstream-owned files.
> Upstream still reads the verdict from `output[0]` (a reasoning item behind
> the GLM gateway), so the judge fails open on this deployment and polly's
> `smart_routing_harness: auto` is inert unless the `llm:` block is pointed
> at a non-reasoning model. Kept below for the reasoning, not as live
> guidance.
...
polly is **not** baked into the server image. It is delivered
to the Fly volume and registered with:

```
OMNIGENT_BUILTIN_AGENT_DIRS=/data/artifacts/polly
```
...
> re-extract from that tarball and restart. The `-size 0` check here is the
> only guard: the server registers whatever the volume holds and fails per
> session, not at boot, so never skip it.
