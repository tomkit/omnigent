# Fork surface atop upstream v0.15.0

How much we've forked, where the cost sits, and what the next upstream upgrade
should take. Measured 2026-09-19 at v0.14.0 (re-checked 2026-09-25 at v0.15.0), after pruning every remaining patch to an
upstream-owned file.

Regenerate any number here with:

```bash
git fetch origin --tags && git diff --numstat v0.15.0..HEAD
```
...
| Fork-owned paths (`deploy/agents`, `deploy/fly`, `tests/deploy`, fork workflow + script) | 17 | +2,119 | none |
